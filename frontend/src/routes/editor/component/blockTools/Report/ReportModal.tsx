...
const AnalystReportModal: React.FC<AnalystReportModalProps> = ({
  createIframe,
  onExit,
}) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.get(
        import.meta.env.VITE_SERVER_REPORT_API_URI,
        {
          params: {
    ...
    <>
      <Modal show={show}>
        ...
        <Modal.Footer>
          ...
          <Button onClick={handleSave}>기업 검색</Button>
        </Modal.Footer>
      </Modal>
      ...
      <Modal show={reportshow} dialogClassName="max-w-4xl mx-auto w-full h-2xl">
        <Modal.Header className="flex flex-col items-start space-y-4">
          <div className="w-full flex justify-between items-center">
            ...
            <Button
              className="bg-white border-none text-black hover:text-black-700"
              onClick={() => {
                setReportshow(false);
                onExit();
              }}
        ...
        >
          ...
          {isMore && (
            <div className=" bg-white py-2">
              <Button className="mt-4 w-full" onClick={searchMore}>
                더 불러 오기
              </Button>
            </div>
          )}
